...
/**
 * Build the system prompt that encodes the scoring rubric and reviewer voice rules.
 * @param {object} params
 * @param {string} params.labTitle
 * @param {string} params.attempt  "1st" | "2nd"
 * @param {string} params.firstName  Learner's first name
 */
export function buildSystemPrompt({ labTitle, attempt, firstName }) {
...
/**
 * Build the user message that includes the learner details, code files, and reviewer notes.
 */
export function buildUserMessage({ learnerName, reviewerName, reviewerNotes, codeFiles }) {
