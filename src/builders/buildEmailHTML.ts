...
export function buildEmailHTML({
  ...
  const ratingLabel = (raw: number): string => {
  ...
  const scoreColor = (raw: number): string =>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 20px;background:#f9fafb;border-bottom:1px solid #e5e7eb;">
        <div style="display:flex;align-items:center;gap:14px;">
          ...
          <div>
            ...
            <div style="font-size:11px;color:#9ca3af;margin-top:2px;text-transform:uppercase;letter-spacing:0.06em;">Weight: ${c.effectiveWeight}%${attempt === '2nd' ? ` &nbsp;&middot;&nbsp; Base: ${c.weight}%` : ''}</div>
          </div>
        </div>
  ...
  const progressPct = Math.min((parseFloat(String(totalScore)) / maxScore) * 100, 100).toFixed(1);
...
<div style="max-width:680px;margin:36px auto;border-radius:16px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,0.18);">
      ...
      <tr>
        <td style="padding:28px 32px;vertical-align:middle;width:60%;">
          ...
          <div style="font-size:12px;color:rgba(255,255,255,0.65);margin-bottom:18px;">
            &Sigma; (raw &divide; 5 &times; weight%) &nbsp;&middot;&nbsp; Max: ${maxScore}%
            ${passed
              ? `&nbsp;&middot;&nbsp;<span style="color:#fff;font-weight:700;">&#10003; PASSED</span>`
              : `&nbsp;&middot;&nbsp;<span style="color:#fff;font-weight:700;">&#10007; NOT PASSED</span>`}
            ${redoLab ? `<br><span style="color:rgba(255,255,255,0.85);font-weight:600;">Re-do Required</span>` : ''}
            ${plagiarism ? `<br><span style="color:rgba(255,255,255,0.85);font-weight:600;">&#9888; Plagiarism Flagged</span>` : ''}
          </div>
  ...
  <div style="background:#f0f2f5;padding:12px 0 0;">
    ...
    <div style="background:#ffffff;margin:0 0 8px;padding:32px 40px;">
      ...
      ${strengths ? `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:18px 20px;margin-bottom:12px;"><div style="font-size:10px;font-weight:700;color:#16a34a;text-transform:uppercase;letter-spacing:0.14em;margin-bottom:8px;">Strengths</div><div style="font-size:13px;color:#374151;line-height:1.8;">${strengths}</div></div>` : ''}
      ${improvements ? `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:18px 20px;margin-bottom:12px;"><div style="font-size:10px;font-weight:700;color:#dc2626;text-transform:uppercase;letter-spacing:0.14em;margin-bottom:8px;">Gaps</div><div style="font-size:13px;color:#374151;line-height:1.8;">${improvements}</div></div>` : ''}
      ${otherRemarks ? `<div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:12px;padding:18px 20px;"><div style="font-size:10px;font-weight:700;color:#7c3aed;text-transform:uppercase;letter-spacing:0.14em;margin-bottom:8px;">Other Remarks</div><div style="font-size:13px;color:#374151;line-height:1.8;">${otherRemarks}</div></div>` : ''}
    </div>` : ''}
    ...
    <div style="background:#ffffff;padding:18px 40px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
      <div style="font-size:11px;color:#9ca3af;">Score formula: &Sigma; (raw &divide; 5 &times; weight%)${attempt === '2nd' ? ' &middot; 2nd attempt max = 80%' : ''}</div>
      <div style="font-size:11px;color:#9ca3af;">AmaliTech &middot; ${dateStrLong}</div>
    </div>
  </div>
</div>
</body></html>`;
